-- lua/context-groups/utils/yaml_pure.lua
-- Pure Lua YAML parser implementation (fallback for when Rust implementation is unavailable)
-- This file preserves the original YAML parsing functionality

-- Import the original YAML parser implementation (copied from the original yaml.lua)
...
function M.parse(yaml_str)
  ...
  local success, result = pcall(function()
    return Parser:new(exports.tokenize(yaml_str)):parse()
  end)
  
  if not success then
    return nil, "YAML parsing failed: " .. tostring(result)
  end
...
function M.encode(tbl, opts)
  ...
  opts = opts or {}
  local block_style = (opts.block_style ~= nil) and opts.block_style or true
  
  local success, result = pcall(function()
    -- This is a simplified implementation of the YAML encoding
    -- For a more complete implementation, an external library would be needed
    ...
    local function format_value(v, indent_str)
      local indent_next = indent_str .. "  "
      
      if type(v) == "table" then
        if is_array(v) then
          local result = {}
          for _, item in ipairs(v) do
            table.insert(result, indent_str .. "- " .. format_value(item, indent_next))
          end
            ...
            if type(val) == "table" then
              ...
              if is_array(val) then
                for _, item in ipairs(val) do
                  table.insert(result, indent_next .. "- " .. format_value(item, indent_next .. "  "))
                end
              else
                for subk, subv in pairs(val) do
                  table.insert(result, indent_next .. subk .. ": " .. format_value(subv, indent_next))
                end
              end
            else
              table.insert(result, indent_str .. k .. ": " .. format_value(val, indent_next))
            end
          end
          return table.concat(result, "\n")
        end
      elseif type(v) == "string" then
        ...
        if v:match("^[0-9]") or v:match(":[%s]") or 
           v == "true" or v == "false" or v == "yes" or v == "no" or
           v == "null" or v == "~" or v == "" or v:match("^%s") or v:match("%s$") then
          return '"' .. v .. '"'
        else
          return v
        end
    ...
    for _, k in ipairs(keys) do
      ...
      if type(v) == "table" then
        if is_array(v) then
          ...
          result[#result + 1] = k .. ":"  
          for _, item in ipairs(v) do
            result[#result + 1] = "  - " .. format_value(item, "  ")
          end
        else
          ...
          for _, nk in ipairs(nested_keys) do
            ...
            if type(nv) == "table" then
              if is_array(nv) then
                ...
                result[#result + 1] = "  " .. nk .. ":"
                local inner_keys = {}
                for ik in pairs(nv) do
                  table.insert(inner_keys, ik)
                end
                table.sort(inner_keys)
                
                for _, ik in ipairs(inner_keys) do
                  local iv = nv[ik]
                  result[#result + 1] = "    " .. ik .. ": " .. format_value(iv, "    ")
                end
              end
            else
              -- Handle simple value
              result[#result + 1] = "  " .. nk .. ": " .. format_value(nv, "  ")
            end
          end
        end
      else
        -- Handle simple root value
        result[#result + 1] = k .. ": " .. format_value(v, "")
      end
    end
  ...
  if not success then
    return nil, "YAML encoding failed: " .. tostring(result)
  end
  
  return result
end

-- For compatibility with the original yaml.lua API
M.eval = M.parse
M.dump = function(tbl) print(M.encode(tbl, { block_style = true })) end
